...
namespace lib60870
{
    ...
    public class ASDU
    {
        ...
        private byte oa; /* originator address  传输原因高字节*/
        ...
        private int faultEventTelesignalisationNum = 0;//故障事件遥信个数;
        ...
        private int faultEventTelemeteringNum = 0;//故障事件遥测个数
        ...
        private int faultEventTelesignalisationType = 0;//故障遥信类型;
        ...
        private int faultEventTelemeteringType = 0;//故障遥测类型
        ...
        public int FaultEventTelemeteringType
        {
            ...
            set { faultEventTelemeteringType = value; }
                                       ...
                                       //                          //传送原因     测试消息     否定的确认     来源地址   公共地址     是否依序
        ...
        public ASDU(ConnectionParameters parameters, byte[] msg, int bufPos, int msgLength)
        {
            ...
            ca = msg[bufPos++];//asdu公共地址

            if (parameters.SizeOfCA > 1)
                ca += (msg[bufPos++] * 0x100);

            if (typeId == TypeID.M_FT_NA_1)//故障事件信息
            {
                FaultEventTelesignalisationNum = msg[bufPos++];//带时标遥信个数
        ...
        /// </summary>
        /// This function add an information object (InformationObject) to the ASDU. NOTE: that all information objects 此函数将信息对象（InformationObject）添加到ASDU。注意：所有信息对象
        /// have to be of the same type. Otherwise an ArgumentException will be thrown.  必须是同一类型。否则将引发ArgumentException。
        /// The function returns true when the information object has been added to the ASDU. The function returns false if
        /// there is no space left in the ASDU to add the information object./将信息对象添加到asdu后，函数返回true。如果asdu中没有剩余空间来添加信息对象。
            ...
            try
            {
                switch (typeId)
                {
                    ...
                    case TypeID.P_AC_NA_1: /* 113 - Parameter for activation */
                        ...
                        break;
                    case TypeID.C_RR_NA_1://读定值区号 201
                    ...
                    case TypeID.C_RS_NA_1:// 读参数和定值 202
                    ...
                    case TypeID.F_FR_NA_1:// 文件传输 210
