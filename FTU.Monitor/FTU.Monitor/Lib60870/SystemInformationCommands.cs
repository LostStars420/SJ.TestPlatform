
using System;

namespace lib60870
{
    #region 总召唤命令 100
    public class QualifierOfInterrogation 
	{	
		public static byte STATION = 20;
		public static byte GROUP_1 = 21;
		public static byte GROUP_2 = 22;
		public static byte GROUP_3 = 23;
		public static byte GROUP_4 = 24;
		public static byte GROUP_5 = 25;
		public static byte GROUP_6 = 26;
		public static byte GROUP_7 = 27;
		public static byte GROUP_8 = 28;
		public static byte GROUP_9 = 29;
		public static byte GROUP_10 = 30;
		public static byte GROUP_11 = 31;
		public static byte GROUP_12 = 32;
		public static byte GROUP_13 = 33;
		public static byte GROUP_14 = 34;
		public static byte GROUP_15 = 35;
		public static byte GROUP_16 = 36;
	}
	public class InterrogationCommand : InformationObject
	{
		override public int GetEncodedSize() {
			return 1;
		}

		override public TypeID Type {
			get {
				return TypeID.C_IC_NA_1;
			}
		}

		override public bool SupportsSequence {
			get {
				return false;
			}
		}

		byte qoi;

		public byte QOI {
			get {
				return this.qoi;
			}
			set {
				qoi = value;
			}
		}

		public InterrogationCommand (int ioa, byte qoi) : base(ioa)
		{
			this.qoi = qoi;
		}

		internal InterrogationCommand (ConnectionParameters parameters, byte[] msg, int startIndex) :
			base(parameters, msg, startIndex, false)
		{
			startIndex += parameters.SizeOfIOA; /* skip IOA */

			qoi = msg [startIndex++];
		}

		internal override void Encode(Frame frame, ConnectionParameters parameters, bool isSequence) {
			base.Encode(frame, parameters, isSequence);

			frame.SetNextByte (qoi);
		}

	}
#endregion

    #region 电能脉冲召唤 101
    public class CounterInterrogationCommand : InformationObject
	{
		override public int GetEncodedSize() {
			return 1;
		}

		override public TypeID Type {
			get {
				return TypeID.C_CI_NA_1;
			}
		}

		override public bool SupportsSequence {
			get {
				return false;
			}
		}

		byte qcc;

		/// <summary>
		/// Gets or sets the QCC (Qualifier of counter interrogation).
		/// </summary>
		/// <value>The QCC</value>
		public byte QCC {
			get {
				return this.qcc;
			}
			set {
				qcc = value;
			}
		}

		public CounterInterrogationCommand (int ioa, byte qoi) : base(ioa)
		{
			this.qcc = qoi;
		}

		internal CounterInterrogationCommand (ConnectionParameters parameters, byte[] msg, int startIndex) :
			base(parameters, msg, startIndex, false)
		{
			startIndex += parameters.SizeOfIOA; /* skip IOA */

			qcc = msg [startIndex++];
		}

		internal override void Encode(Frame frame, ConnectionParameters parameters, bool isSequence) {
			base.Encode(frame, parameters, isSequence);

			frame.SetNextByte (qcc);
		}

	}

#endregion

    #region 读命令 102
    public class ReadCommand : InformationObject
	{
		override public int GetEncodedSize() {
			return 0;
		}

		override public TypeID Type {
			get {
                return TypeID.C_RD_NA_1;// 读命令 102
			}
		}

		override public bool SupportsSequence {
			get {
				return false;
			}
		}

		public ReadCommand (int ioa) : base(ioa)
		{
		}

		internal ReadCommand (ConnectionParameters parameters, byte[] msg, int startIndex) :
			base(parameters, msg, startIndex, false)
		{
		}
	}
    #endregion

    #region 测试命令 104
    public class TestCommand : InformationObject
	{
		override public int GetEncodedSize() {
			return 2;
		}

		override public TypeID Type {
			get {
				return TypeID.C_TS_NA_1;
			}
		}

		private bool valid = true;

		public bool Valid {
			get {
				return valid;
			}
		}

		override public bool SupportsSequence {
			get {
				return false;
			}
		}

		public TestCommand () : base(0)
		{
		}

		internal TestCommand (ConnectionParameters parameters, byte[] msg, int startIndex) :
		base(parameters, msg, startIndex, false)
		{
			startIndex += parameters.SizeOfIOA; /* skip IOA */

			if (msg [startIndex++] != 0xaa)
				valid = false;

			if (msg [startIndex] != 0x55)
				valid = false;
		}

		internal override void Encode(Frame frame, ConnectionParameters parameters, bool isSequence) {
			base.Encode(frame, parameters, isSequence);

			frame.SetNextByte (0xaa);
			frame.SetNextByte (0x55);
		}
	}
    #endregion

    #region 时钟同步命令 103
    public class ClockSynchronizationCommand : InformationObject
	{
		override public int GetEncodedSize() {
			return 7;
		}

		override public TypeID Type {
			get {
				return TypeID.C_CS_NA_1;
			}
		}

		override public bool SupportsSequence {
			get {
				return false;
			}
		}

		private CP56Time2a newTime;

		public CP56Time2a NewTime {
			get {
				return this.newTime;
			}
			set {
				newTime = value;
			}
		}

		public ClockSynchronizationCommand (int ioa, CP56Time2a newTime) : base(ioa)
		{
			this.newTime = newTime;
		}

		internal ClockSynchronizationCommand (ConnectionParameters parameters, byte[] msg, int startIndex) :
			base(parameters, msg, startIndex, false)
		{
			startIndex += parameters.SizeOfIOA; /* skip IOA */

			/* parse CP56Time2a (time stamp) */
			newTime = new CP56Time2a (msg, startIndex);
		}

		internal override void Encode(Frame frame, ConnectionParameters parameters, bool isSequence) {
			base.Encode(frame, parameters, isSequence);

			frame.AppendBytes (newTime.GetEncodedValue ());
		}
	}
#endregion

    #region 复位进程命令 105
    public class ResetProcessCommand : InformationObject
	{
		override public int GetEncodedSize() {
			return 1;
		}

		override public TypeID Type {
			get {
				return TypeID.C_RP_NA_1;
			}
		}

		override public bool SupportsSequence {
			get {
				return false;
			}
		}

		byte qrp;

		/// <summary>
		/// Gets or sets the QRP (Qualifier of reset process command).
		/// </summary>
		/// <value>The QRP</value>
		public byte QRP {
			get {
				return this.qrp;
			}
			set {
				qrp = value;
			}
		}

		public ResetProcessCommand (int ioa, byte qrp) : base(ioa)
		{
			this.qrp = qrp;
		}

		internal ResetProcessCommand (ConnectionParameters parameters, byte[] msg, int startIndex) :
			base(parameters, msg, startIndex, false)
		{
			startIndex += parameters.SizeOfIOA; /* skip IOA */

			qrp = msg [startIndex++];
		}

		internal override void Encode(Frame frame, ConnectionParameters parameters, bool isSequence) {
			base.Encode(frame, parameters, isSequence);

			frame.SetNextByte (qrp);
		}

	}
    #endregion 

    #region 延时获得命令 106
    public class DelayAcquisitionCommand : InformationObject
	{
		override public int GetEncodedSize() {
			return 2;
		}

		override public TypeID Type {
			get {
				return TypeID.C_CD_NA_1;
			}
		}

		override public bool SupportsSequence {
			get {
				return false;
			}
		}

		private CP16Time2a delay;

		public CP16Time2a Delay {
			get {
				return this.delay;
			}
			set {
				delay = value;
			}
		}

		public DelayAcquisitionCommand (int ioa, CP16Time2a delay) : base(ioa)
		{
			this.delay = delay;
		}

		internal DelayAcquisitionCommand (ConnectionParameters parameters, byte[] msg, int startIndex) :
			base(parameters, msg, startIndex, false)
		{
			startIndex += parameters.SizeOfIOA; /* skip IOA */

			/* parse CP16Time2a (time stamp) */
			delay = new CP16Time2a (msg, startIndex);
		}

		internal override void Encode(Frame frame, ConnectionParameters parameters, bool isSequence) {
			base.Encode(frame, parameters, isSequence);

			frame.AppendBytes (delay.GetEncodedValue ());
		}
       
	}
     #endregion

    #region 切换定值区命令 200
    public class ChangeSettingAreaCommand : InformationObject
    {
        override public int GetEncodedSize()
        {
            return 2;
        }

        override public TypeID Type
        {
            get
            {
                return TypeID.C_SR_NA_1;
            }
        }

        override public bool SupportsSequence
        {
            get
            {
                return false;
            }
        }

        int  settingArea;

        public int  SettingArea
        {
            get
            {
                return this.settingArea;
            }
            set
            {
                settingArea = value;
            }
        }

        public ChangeSettingAreaCommand(int ioa, int settingArea)
            : base(ioa)
        {
            this.settingArea =settingArea;
        }

        internal ChangeSettingAreaCommand(ConnectionParameters parameters, byte[] msg, int startIndex) :
            base(parameters, msg, startIndex, false)
        {
            startIndex += parameters.SizeOfIOA; /* skip IOA */

            settingArea = msg[startIndex++];
        }

        internal override void Encode(Frame frame, ConnectionParameters parameters, bool isSequence)
        {
            base.Encode(frame, parameters, isSequence);

            frame.SetNextByte((byte)settingArea);
            frame.SetNextByte((byte)(settingArea>>8));
        }

    }
    #endregion

    #region 读定值区号 201
    public class ReadParameterAreaCommand : InformationObject
    {
        override public int GetEncodedSize()
        {
            return 0;
        }
        /// <summary>
        /// 当前定值区号
        /// </summary>
        private int _SN1;

        public int SN1
        {
            get { return _SN1; }
            set { _SN1 = value; }
        }
        /// <summary>
        /// 最小定值区号
        /// </summary>
        private int _SN2;

        public int SN2
        {
            get { return _SN2; }
            set { _SN2 = value; }
        }
        /// <summary>
        /// 最大定值区号
        /// </summary>
        private int _SN3;

        public int SN3
        {
            get { return _SN3; }
            set { _SN3 = value; }
        }
        
        override public TypeID Type
        {
            get
            {
                return TypeID.C_RR_NA_1;
            }
        }

        override public bool SupportsSequence
        {
            get
            {
                return false;
            }
        }
        public ReadParameterAreaCommand(int ioa): base(ioa)
        {            
        }

        internal ReadParameterAreaCommand(ConnectionParameters parameters, byte[] msg, int startIndex) :
            base(parameters, msg, startIndex, false)
        {

            startIndex += parameters.SizeOfIOA; /* skip IOA */

           

            SN1 = msg[startIndex++] + (msg[startIndex++] <<8);
            SN2 = msg[startIndex++] + (msg[startIndex++] << 8);
            SN3 = msg[startIndex++] + (msg[startIndex++] << 8);
        }

        internal override void Encode(Frame frame, ConnectionParameters parameters, bool isSequence)
        {
            base.Encode(frame, parameters, isSequence);

           
        }

    }
    #endregion

    #region 读定值 202
    public class ReadParameterCommand : InformationObject
    {
        override public int GetEncodedSize()
        {
            return buf.Length;
        }
        /// <summary>
        /// 当前定值区号
        /// </summary>
        private int _SN1;

        public int SN1
        {
            get { return _SN1; }
            set { _SN1 = value; }
        }
        private byte[] buf;

        public byte[] Buf
        {
            get { return buf; }
            set { buf = value; }
        }
        
        override public TypeID Type
        {
            get
            {
                return TypeID.C_RS_NA_1;
            }
        }

        override public bool SupportsSequence
        {
            get
            {
                return false;
            }
        }
        public ReadParameterCommand(byte[] buf)
        {
            this.buf = buf;
        }

        //internal ReadParameterCommand(ConnectionParameters parameters, byte[] msg, int startIndex) :
        //    base(parameters, msg, startIndex, false)
        //{
        //    startIndex += parameters.SizeOfIOA; /* skip IOA */
        //    SN1 = msg[startIndex++] + msg[startIndex++] << 8;
          
        //}

        internal override void Encode(Frame frame, ConnectionParameters parameters, bool isSequence)
        {
            foreach (byte b in buf)
            {
                frame.SetNextByte(b);
            }
        }

    }
    #endregion

    #region 写参数和定值 203
    public class SetParameterCommand : InformationObject
    {
        override public int GetEncodedSize()
        {
            return buf.Length;
        }
        /// <summary>
        /// 当前定值区号
        /// </summary>
        private int _SN1;

        public int SN1
        {
            get { return _SN1; }
            set { _SN1 = value; }
        }
        private byte[] buf;

        public byte[] Buf
        {
            get { return buf; }
            set { buf = value; }
        }

        override public TypeID Type
        {
            get
            {
                return TypeID.C_WS_NA_1;
            }
        }

        override public bool SupportsSequence
        {
            get
            {
                return false;
            }
        }
        public SetParameterCommand(byte[] buf)
        {
            this.buf = buf;
        }

        //internal ReadParameterCommand(ConnectionParameters parameters, byte[] msg, int startIndex) :
        //    base(parameters, msg, startIndex, false)
        //{
        //    startIndex += parameters.SizeOfIOA; /* skip IOA */
        //    SN1 = msg[startIndex++] + msg[startIndex++] << 8;

        //}

        internal override void Encode(Frame frame, ConnectionParameters parameters, bool isSequence)
        {
            foreach (byte b in buf)
            {
                frame.SetNextByte(b);
            }
        }

    }
    #endregion

    #region 文件传输 210
    public class FileServiceCommand : InformationObject
    {
        override public int GetEncodedSize()
        {
            return buf.Length;
        }
       
        private byte[] buf;

        public byte[] Buf
        {
            get { return buf; }
            set { buf = value; }
        }

        override public TypeID Type
        {
            get
            {
                return TypeID.F_FR_NA_1;
            }
        }

        override public bool SupportsSequence
        {
            get
            {
                return false;
            }
        }
        public FileServiceCommand(byte[] buf)
        {
            this.buf = buf;
        }       

        internal override void Encode(Frame frame, ConnectionParameters parameters, bool isSequence)
        {
            foreach (byte b in buf)
            {
                frame.SetNextByte(b);
            }
        }

    }
    #endregion

    #region 软件升级命令 211
    public class UpdataCommand : InformationObject
    {
        override public int GetEncodedSize()
        {
            return 1;
        }

        override public TypeID Type
        {
            get
            {
                return TypeID.F_SR_NA_1;
            }
        }

        override public bool SupportsSequence
        {
            get
            {
                return false;
            }
        }

        byte ctype;

        public byte Ctype
        {
            get
            {
                return this.ctype;
            }
            set
            {
                ctype = value;
            }
        }

        public UpdataCommand(int ioa, byte ctype)
            : base(ioa)
        {
            this.ctype = ctype;
        }

        internal UpdataCommand(ConnectionParameters parameters, byte[] msg, int startIndex) :
            base(parameters, msg, startIndex, false)
        {
            startIndex += parameters.SizeOfIOA; /* skip IOA */

            ctype = msg[startIndex++];
        }

        internal override void Encode(Frame frame, ConnectionParameters parameters, bool isSequence)
        {
            base.Encode(frame, parameters, isSequence);

            frame.SetNextByte(ctype);
        }

    }
    #endregion
}

